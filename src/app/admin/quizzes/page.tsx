...
import type { QuizStatus } from "@/lib/contracts/quiz";
...
export default function AdminQuizzesPage() {
  ...
  async function handleGenerateDrafts(event: FormEvent<HTMLFormElement>) {
    ...
    if (!trimmedSecret || !isValidDraftCount(count) || draftStatus === "loading") {
      return;
    }
  ...
  async function handleSaveDraft(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const trimmedSecret = adminSecret.trim();
    if (!trimmedSecret || !activeDraft || saveStatus === "loading") {
      return;
    }

    const update = buildAdminQuizUpdateRequest(activeDraft);
    if (!update) {
      setMessage("Fill every quiz field and select exactly one answer.");
      return;
    }
    ...
    try {
      const response = await updateAdminQuiz(
        trimmedSecret,
        activeDraft.id,
        update,
      );
      ...
      setMessage(`Saved as ${formatStatus(response.quiz.status)}.`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Quiz update failed.");
    } finally {
      setSaveStatus("idle");
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-4 py-4 sm:px-6 sm:py-6 lg:px-8">
        ...
        {message ? (
          <div
            className="rounded-md border border-[var(--line)] bg-[var(--panel)] px-3 py-2 text-sm text-[var(--muted)]"
            role="status"
          >
            {message}
          </div>
                  ...
                  drafts.map((draft, index) => (
                    ...
                    >
                      <span className="text-xs font-semibold text-[var(--muted)]">
                        Draft {index + 1} / {formatStatus(draft.status)}
                      </span>
              ...
              {activeDraft ? (
                <div className="mt-4 grid gap-4">
                  <div className="grid gap-3 sm:grid-cols-3">
                    <Field label="Status" htmlFor="draft-status">
                      <select
                        className="h-11 w-full rounded-md border border-[var(--line)] bg-white px-3 text-sm"
                        id="draft-status"
                        onChange={(event) =>
                  ...
                  <Field label="Question" htmlFor="draft-question">
                    <input
                      ...
                      id="draft-question"
                      onChange={(event) =>
                        updateActiveDraft({ questionEn: event.target.value })
                      }
                  ...
                  <Field label="Korean sentence" htmlFor="draft-sentence">
                    <textarea
                      ...
                      id="draft-sentence"
                      onChange={(event) =>
                        updateActiveDraft({ sentenceKo: event.target.value })
                      }
                  ...
                  <Field label="Review note" htmlFor="draft-review-note">
                    <textarea
                      className="min-h-20 w-full resize-y rounded-md border border-[var(--line)] bg-white p-3 text-sm leading-6 outline-none focus:border-[var(--accent)]"
                      id="draft-review-note"
                      onChange={(event) =>
                        updateActiveDraft({ reviewNote: event.target.value })
                      }
                      value={activeDraft.reviewNote}
                    />
                  </Field>
                  ...
                  <button
                    ...
                    type="submit"
                  >
                    {saveStatus === "loading" ? "Saving..." : "Save review"}
                  </button>
                </div>
              ) : (
...
function formatStatus(status: QuizStatus) {
