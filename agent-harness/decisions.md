...
- Target entity primary keys use PostgreSQL `BIGINT GENERATED BY DEFAULT AS IDENTITY` and JPA `GenerationType.IDENTITY`. Composite usage/stat keys and Spring Session's own string identifiers remain exceptions.
- Treat entity IDs as opaque strings in JSON so JavaScript clients do not depend on numeric precision. Auth accepts UUID strings during the Next-to-Spring transition and emits decimal strings from Spring.
...
- The UUID-to-BIGINT Flyway migration is a cutover boundary: do not apply it while the UUID-based Next backend can still write. Create a Neon restore point immediately before applying it; rollback afterward restores the database as well as the application route.
